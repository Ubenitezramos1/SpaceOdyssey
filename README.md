...
<b> It is highly recommended to configure Git to your code editor of choice, as commits would otherwise have to be done through Vim which is difficult to navigate.</b> Here are some commands to run so Git is configured with these editors. If there's another editor outside of these you want to use, reach out to me.
<ul>
...
<ol>
<li> Use the command line (or Git bash) to go into the folder you'd like the repository cloned in. For example, say I have a folder called "Operating Systems", so the command I would use is this for the command line: <code>cd "C:Web Programming\Space Odyssey"</code> and this in Git Bash: <code>cd /c/Web\ Programming/Space\ Odyssey/</code></li>
<li> Use this command on the command line when you've entered the desired folder: <code>git clone https://github.com/Ubenitezramos1/SpaceOdyssey.git</code></li>
